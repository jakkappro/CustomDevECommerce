...
public class MailMode : IStarterService
{
        ...
        IEnumerable<GetPrehomeFeed.SHOPSHOPITEM> items)
    {
        ...
        if (!readOnly) await _orderService.CreateOrder(ExpandoToPohodaOrder.Map(order, id, items));
        ...
        if (!items.Exists(i => i.ITEM_ID == 320751))
            items.Add(
                new GetPrehomeFeed.SHOPSHOPITEM
                {
                    ITEM_ID = 320751,
                    PRODUCTNAME =
                        "Doplnkový set obrázkov magic mags zelený traktor k aktovkám grade, space, cloud, 2v1",
                    EAN = "4047443358301",
                    DEALER = "JUNIOR",
                    IMGURL = "",
                    PRICE_VAT = 13.86m,
